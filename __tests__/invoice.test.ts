...
describe("Invoice totals", () => {
  test("falls back to a single line from service + price when no lineItems", () => {
    ...
    expect(totals.lineItems[0]).toEqual({
  ...
  test("strips the internal notes/meta suffix from the fallback line label", () => {
    const totals = computeInvoiceTotals({
      service: "Window Cleaning, Gutter Cleaning | Notes: gate code 4455",
      price: 250,
    });
    expect(totals.lineItems[0].description).toBe(
      "Window Cleaning, Gutter Cleaning",
    );
    expect(totals.subtotal).toBe(250);
  });

  test("invoiceItems are presentational and never affect totals", () => {
    const totals = computeInvoiceTotals({
      service: "Full Exterior Package",
      price: 300,
      taxRate: 13,
      invoiceItems: ["Windows", "Gutters", "Driveway pressure wash"],
    });
    expect(totals.lineItems).toHaveLength(1);
    expect(totals.subtotal).toBe(300);
    expect(totals.total).toBe(339);
  });
...
describe("isOverdue", () => {
  test("true when unpaid and past the due date", () => {
    expect(
      isOverdue({
        status: "INVOICED",
        invoicedAt: daysAgo(PAYMENT_TERMS_DAYS + 1),
      }),
    ).toBe(true);
    expect(
      isOverdue({
        status: "UNPAID",
        invoicedAt: daysAgo(PAYMENT_TERMS_DAYS + 5),
      }),
    ).toBe(true);
  });
  test("false before the due date", () => {
    expect(isOverdue({ status: "INVOICED", invoicedAt: daysAgo(1) })).toBe(
      false,
    );
  });
  test("false for paid or non-invoice statuses", () => {
    expect(isOverdue({ status: "PAID", invoicedAt: daysAgo(100) })).toBe(false);
    expect(isOverdue({ status: "SCHEDULED", invoicedAt: daysAgo(100) })).toBe(
      false,
    );
  });
...
describe("isReminderDue", () => {
  ...
  test("sends the first reminder for an overdue invoice", () => {
    expect(isReminderDue({ status: "UNPAID", invoicedAt: overdueAt() })).toBe(
      true,
    );
  });
  ...
  test("never reminds a non-overdue invoice", () => {
    expect(isReminderDue({ status: "INVOICED", invoicedAt: daysAgo(1) })).toBe(
      false,
    );
  });
});
