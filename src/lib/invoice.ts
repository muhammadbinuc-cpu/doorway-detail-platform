...
export interface InvoiceJobLike {
  ...
  taxRate?: number;
  lineItems?: LineItem[];
  // Display-only list of included items shown under a single lump price.
  // Purely presentational — never affects the computed totals.
  invoiceItems?: string[];
}
...
export function normalizeLineItems(job: InvoiceJobLike): LineItem[] {
  ...
  // Lump fallback: use the part of `service` before the internal " | " meta
  // suffix so customer notes (e.g. "| Notes: gate code 4455") never leak onto
  // the official invoice as the line description.
  const lumpLabel = (job.service || "Cleaning Service").split(" | ")[0].trim();
  return [
    {
      description: lumpLabel || "Cleaning Service",
      quantity: 1,
...
export function isReminderDue(
  ...
  return (
    now.getTime() - job.lastReminderAt.getTime() >= REMINDER_GAP_DAYS * DAY_MS
  );
}
