...
import { Loader2, MapPin, Phone, Trash2, FileText, TrendingUp, Settings, X, Truck, Repeat } from "lucide-react";
...
export default function AdminPage() {
    ...
    const handleDelete = async (id: string) => {
        if (!(await confirm({ title: "Delete job?", message: "This permanently removes the job.", danger: true, confirmText: "Delete" }))) return;
        const res = await deleteJob(id);
        if (res.success) toast.success("Job deleted"); else toast.error(errText(res));
    };

    const handleStatusUpdate = async (id: string, status: string) => { const res = await updateJobStatus(id, status); if (!res.success) toast.error(errText(res)); };
    const handlePrice = async (id: string, val: string) => { await updateJobDetails(id, { price: parseFloat(val) }); };
    const handleSendInvoice = async (id: string) => { if (!(await confirm({ message: "Send the invoice email to this customer?", confirmText: "Send" }))) return; const res = await emailInvoice(id); if (res.success) toast.success("Invoice sent"); else toast.error(errText(res)); };
    const handleOnMyWay = async (id: string) => { if (!(await confirm({ message: "Send an 'on my way' SMS to this customer?", confirmText: "Send" }))) return; const res = await sendOnMyWay(id); if (res.success) toast.success("SMS sent"); else toast.error(errText(res)); };
    const handleRecurring = async (id: string) => { if (!(await confirm({ message: "Create a new recurring job from this one?", confirmText: "Create" }))) return; const res = await createRecurringJob(id); if (res.success) toast.success("Recurring job created"); else toast.error(errText(res)); };
                            ...
                            {jobs.map((job) => (
                                <motion.div key={job.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} whileHover={{ scale: 1.02, y: -4 }} className="bg-white p-6 rounded-3xl shadow-sm border flex flex-col md:flex-row justify-between gap-6">
                                    <div className="space-y-2 flex-1">
                                        <div className="flex items-center gap-3 flex-wrap"><h3 className="text-xl font-bold">{job.name || 'Unknown'}</h3><select value={job.status} onChange={(e) => handleStatusUpdate(job.id, e.target.value)} className="px-3 py-1 text-xs font-bold rounded bg-gray-100 border-none outline-none cursor-pointer hover:bg-gray-200 transition-colors">{getNextStatuses(job.status).map((s) => (<option key={s} value={s}>{statusLabel(s)}</option>))}</select>{job.invoiceNumber && <span className="px-2 py-1 text-xs font-mono font-bold text-gray-400 bg-gray-50 rounded">Invoice {formatInvoiceNumber(job.invoiceNumber, job.id)}</span>}</div>
                        ...
                        <div className="space-y-4 max-h-[70vh] overflow-auto pr-1">
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <button type="button" onClick={() => setEditLineItems([...editLineItems, emptyLineItem()])} className="text-xs font-bold text-[#D4AF37] hover:underline">+ Add item</button>
                                </div>
                                {editLineItems.length === 0 && (
                                    <p className="text-xs text-gray-400 mb-2">No line items — the invoice uses the job&apos;s single price. Add items for a detailed breakdown.</p>
                                )}
                                <div className="space-y-2">
                                    {editLineItems.map((li, idx) => (
                                        <div key={idx} className="flex gap-2 items-center">
                                            <input value={li.description} onChange={(e) => setEditLineItems(editLineItems.map((x, i) => i === idx ? { ...x, description: e.target.value } : x))} placeholder="Description" className="flex-1 bg-gray-50 p-2 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-[#D4AF37]" />
                                            <input type="number" min={1} value={li.quantity} onChange={(e) => setEditLineItems(editLineItems.map((x, i) => i === idx ? { ...x, quantity: parseInt(e.target.value) || 1 } : x))} className="w-14 bg-gray-50 p-2 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-[#D4AF37]" title="Qty" />
                                            <input type="number" min={0} value={li.unitPrice} onChange={(e) => setEditLineItems(editLineItems.map((x, i) => i === idx ? { ...x, unitPrice: parseFloat(e.target.value) || 0 } : x))} className="w-20 bg-gray-50 p-2 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-[#D4AF37]" title="Unit price ($)" />
                                            <button type="button" onClick={() => setEditLineItems(editLineItems.filter((_, i) => i !== idx))} className="text-gray-300 hover:text-red-500 transition-colors"><X size={16} /></button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <button onClick={async () => {
                                const cleaned = editLineItems
                                    .filter((li) => li.description.trim() && li.quantity > 0)
                                    .map((li) => ({ description: li.description.trim(), quantity: li.quantity, unitPrice: li.unitPrice }));
                                const res = await updateJobDetails(editingJob.id, { ...editForm, lineItems: cleaned });
                                if (!res.success) { toast.error(errText(res)); return; }
                                toast.success("Invoice settings saved");
                                setShowSettingsModal(false);
                            }} className="w-full bg-black text-white p-4 rounded-xl font-bold hover:bg-[#D4AF37] hover:text-black transition-all mt-4">Save Settings</button>
                        </div>
                    </motion.div>
                </div>
            )}
            ...
            {isScheduleModalOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
                        ...
                        <div className="flex gap-4"><button onClick={() => { setIsScheduleModalOpen(false); setSyncError(null); }} className="flex-1 bg-gray-100 text-gray-500 py-3 rounded-xl font-bold hover:bg-gray-200 transition-colors">Cancel</button><button onClick={async () => { if (schedulingJobId && scheduleDate) { setSyncingJobId(schedulingJobId); setSyncError(null); const result = await confirmBooking(schedulingJobId, scheduleDate); setSyncingJobId(null); if (result.success) { toast.success("Job scheduled"); setIsScheduleModalOpen(false); } else { setSyncError('error' in result ? (result.error ?? 'Unknown error') : 'Unknown error'); } } else { toast.error("Please select a date."); } }} disabled={!!syncingJobId} className="flex-1 bg-black text-white py-3 rounded-xl font-bold hover:bg-[#D4AF37] hover:text-black transition-colors disabled:opacity-50">{syncingJobId ? <span className="flex items-center justify-center gap-2"><Loader2 className="animate-spin" size={16} /> Syncing...</span> : "Confirm"}</button></div>
                    </motion.div>
                </div>
            )}
        </div>
